...
void DDlightEventAction::BeginOfEventAction(const G4Event* evt) 
{
  ...
  if (!runAct) 
    runAct = 
      dynamic_cast<const DDlightRunAction*>
      (G4RunManager::GetRunManager()->GetUserRunAction());
  
  if (!genAction)
    genAction = dynamic_cast<const DDlightPrimaryGeneratorAction*>
...
void DDlightEventAction::EndOfEventAction(const G4Event* evt) {
  ...
  std::cout << "DEBUG: DDlightEventAction::EndOfEventAction" << std::endl;
