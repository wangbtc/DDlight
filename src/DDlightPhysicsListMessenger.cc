...
DDlightPhysicsListMessenger::
  DDlightPhysicsListMessenger(DDlightPhysicsList* pPhys) 
  : G4UImessenger(),
    fPhysicsList(pPhys)
{
  fDDlightDir = new G4UIdirectory("/DDlight/");
  fDDlightDir->SetGuidance("UI commands of this example");

  fPhysDir = new G4UIdirectory("/DDlight/phys/");
  fPhysDir->SetGuidance("PhysicsList control");
 
  fVerboseCmd = new G4UIcmdWithAnInteger("/DDlight/phys/verbose",this);
  ...
  fCerenkovCmd =
           new G4UIcmdWithAnInteger("/DDlight/phys/cerenkovMaxPhotons",this);
...
DDlightPhysicsListMessenger::~DDlightPhysicsListMessenger()
{
  delete fVerboseCmd;
  delete fCerenkovCmd;
  delete fPhysDir;
...
void DDlightPhysicsListMessenger::SetNewValue(G4UIcommand* command,
                                               G4String newValue)
{
